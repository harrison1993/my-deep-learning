...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            #print("final_inputs",np.shape(final_inputs))
            # TODO: Output error - Replace this value with your calculations.
            #error = (y-final_outputs)*final_outputs*(1-final_outputs) # Output layer error is the difference between desired target and actual output.
            error = y-final_outputs
            # TODO: Calculate the hidden layer's contribution to the error

            
            
            #hidden_outputs_prime = np.dot(hidden_outputs[:,None],(1-hidden_outputs)[None,:])
            
            #print("self.weights_hidden_to_output",np.shape(self.weights_hidden_to_output.T))
            #print("hidden_outputs",np.shape(hidden_outputs[:,None]))
            #hidden_outputs_prime = np.dot(self.weights_hidden_to_output.T,hidden_outputs[:,None])
            hidden_error = error*self.weights_hidden_to_output.T#np.dot(hidden_outputs_prime,hidden_outputs[:,None].T)
            #print(hidden_error)
            ...
            # hidden_error_term = np.dot(np.dot(hidden_error,hidden_outputs[:,None]),(1-hidden_outputs[:,None].T))#*hidden_outputs*(1-hidden_outputs)
            hidden_error_term = hidden_error * hidden_outputs * (1-hidden_outputs)#这里可以直接乘而不需要np.dot()??
            ...
            print("hidden_error_term.T",hidden_error_term.T)
            print("hidden_error_term.T",np.shape(hidden_error_term.T))
            print("hidden_error_term[:,None]",hidden_error_term[:,None])
            print("hidden_error_term[:,None]",np.shape(hidden_error_term[:,None]))
            print("hidden_outputs[:,None]",np.shape(hidden_outputs[:,None].T))
            '''
            
            #delta_weights_i_h += np.dot(X[:,None],hidden_error_term)
            delta_weights_i_h += (np.dot(hidden_error_term.T,X[:,None].T)).T
            #print("delta_weights_i_h",np.shape(delta_weights_i_h))
            # Weight step (hidden to output)           
            #print("delta_weights_h_o",np.shape(delta_weights_h_o))
            delta_weights_h_o +=output_error_term*hidden_outputs[:,None]#np.dot(output_error_term,hidden_inputs)# np.dot(output_error_term,hidden_inputs[None ,:])
            #print("hidden_inputs[:,None]",np.shape(hidden_inputs[:,None]))
            #print("hidden_inputs[:,None]",np.shape( hidden_inputs[:,None]))
...
hidden_nodes = 65#由于模型结构较大，需要使用较低的学习率来保证训练稳定性，进而影响训练效率;！
output_nodes = 1
'''

iterations = 1000
learning_rate = 0.7#学习率的选取较小，模型训练效率较低：在当前隐藏层单元不大的情况下，0.5〜1.0的学习率能在保证训练稳定性的同时提高时间效率。
hidden_nodes = 12
output_nodes = 1
